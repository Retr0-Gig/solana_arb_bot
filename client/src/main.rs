...
use client::pool_utils::stable::convert;
...
fn main() {
    ...
    let file = File::open("../keypair.json").unwrap();
    let reader = std::io::BufReader::new(file);
    let seed: Seed = serde_json::from_reader(reader).unwrap();

    let seed_byte = convert(seed.seed_phrase.as_str());

    env_logger::init();


    // ** setup RPC connection
    let connection_url = seed.rpc_cluster.as_str();
    ...
    // setup anchor things
    let owner = keypair_from_seed_and_derivation_path(&seed_byte, None).unwrap();
